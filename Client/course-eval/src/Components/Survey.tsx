import React from "react";
import { Formik } from "formik";
import { Button, FormGroup } from "semantic-ui-react";
import { Radio, TextArea, Form } from "formik-semantic-ui-react";

interface Values {
  courseSelection: string;
  courseComments: string;
  materialSelection: string;
  materialComments: string;
}

export const Survey: React.FC = () => {
  const initialValues: Values = {
    courseSelection: "",
    courseComments: "",
    materialSelection: "",
    materialComments: "",
  };
  return (
    <div>
      <h1 style={{ textAlign: "center" }}>Course name</h1>
      <h2 style={{ textAlign: "center", color: "grey" }}>instructor name</h2>

      <Formik
        initialValues={initialValues}
        onSubmit={(data, { setSubmitting }) => {
          setSubmitting(true);
          // make async call

          fetch("http://localhost:8000/surveyResponse", {
            method: "POST",
            headers: {
              Accept: "application/json",
              "Content-Type": "application/json",
            },
            body: JSON.stringify(data),
          });
          console.log("submit: ", data);
          setSubmitting(false);
        }}>
        {({ values, handleSubmit, isSubmitting }) => (
          <Form onSubmit={handleSubmit}>
            <h3>Instructor</h3>
            <div>
              <FormGroup inline>
                <label>
                  On a scale from 1 to 5, how satisfied are
                  <br /> you with your instructor for this course?
                </label>
                <Radio name="courseSelection" value="1" label="1" />
                <Radio name="courseSelection" value="2" label="2" />
                <Radio name="courseSelection" value="3" label="3" />
                <Radio name="courseSelection" value="4" label="4" />
                <Radio name="courseSelection" value="5" label="5" />
              </FormGroup>
            </div>
            <TextArea
              placeholder="Input text here"
              style={{ minHeight: 100 }}
              name="courseComments"
            />
            <h3>Course</h3>
            <FormGroup>
              <label>
                On a scale from 1 to 5, how satisfied are <br />
                you with the course material?
              </label>
              <Radio name="materialSelection" label="1" value="1" />
              <Radio name="materialSelection" label="2" value="2" />
              <Radio name="materialSelection" label="3" value="3" />
              <Radio name="materialSelection" label="4" value="4" />
              <Radio name="materialSelection" label="5" value="5" />
            </FormGroup>
            <TextArea
              placeholder="Input text here"
              style={{ minHeight: 100 }}
              name="materialComments"
            />

            <Button
              disabled={isSubmitting}
              name="Submit"
              id="button-submit"
              type="submit">
              Submit
            </Button>
            <pre>{JSON.stringify(values, null, 2)}</pre>
          </Form>
        )}
      </Formik>
    </div>
  );
};
